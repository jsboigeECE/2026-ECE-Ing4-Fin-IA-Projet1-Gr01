...
Ce projet a pour objectif de générer automatiquement le calendrier d’un championnat sportif de type round-robin aller-retour,
chaque équipe rencontre toutes les autres deux fois, une à domicile et une à l’extérieur,
tout en respectant un ensemble de contraintes réalistes issues de la recherche opérationnelle.
Le problème est modélisé comme un problème de satisfaction de contraintes CSP et résolu à l’aide du solveur CP-SAT d’OR-Tools.
L’optimisation porte principalement sur la minimisation des "breaks",c’est-à-dire les enchaînements de matchs consécutifs joués au même endroit deux domiciles ou deux déplacements d’affilée.
...
•	Respect des contraintes suivantes :
  o	Une équipe joue exactement un match par journée
  o	Chaque paire d’équipes se rencontre exactement deux fois
  o	Équilibre parfait entre matchs à domicile et à l’extérieur
  o	Limitation et minimisation des breaks
•	Optimisation du calendrier selon un critère global
•	Affichage détaillé :
  o	Calendrier par journée
  o	Vue par équipe
  o	Statistiques domicile / extérieur et breaks
•	Visualisation graphique du calendrier 
...
•	La vérification automatique des contraintes est imposées par le solveur CP-SAT
•	L’affichage des statistiques finales :
  o	Nombre de matchs domicile / extérieur par équipe
  o	Nombre total de breaks
